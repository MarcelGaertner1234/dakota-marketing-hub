...
type SizeFormat = "auto" | "1024x1024" | "1536x1024" | "1024x1536"

const FORMAT_LABELS: Record<Exclude<SizeFormat, "auto">, string> = {
  "1024x1024": "Quadrat (Instagram)",
  "1536x1024": "Landscape (Facebook)",
  "1024x1536": "Portrait (TikTok / Stories)",
}
...
/**
 * KI-Illustration Modal für Social Media Posts.
 * Geklont aus dem Tischkarten-Modal mit Format-Override-Picker.
 */
export function SocialIllustrationModal({
  postId,
  postTitle,
  platform,
  open,
  ...
  const [sizeFormat, setSizeFormat] = useState<SizeFormat>("auto")
  ...
  const autoFormatLabel =
    PLATFORM_DEFAULT[platform] && sizeFormat === "auto"
      ? `Auto (${FORMAT_LABELS[PLATFORM_DEFAULT[platform] as Exclude<SizeFormat, "auto">]})`
      : "Auto"
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (sizeFormat !== "auto") formData.set("size", sizeFormat)

      const res = await fetch(
        `/api/social/${postId}/generate-illustration`,
    ...
    >
      ...
      >
        ...
        <p className="mb-5 text-sm text-gray-500 dark:text-gray-400">
          ...
          <span className="capitalize">{platform}</span> — im Dakota-Stil
          via GPT Image 1.5.
        </p>
        ...
        {resultUrl ? (
          <div className="space-y-4">
            ...
            <p className="text-center text-xs text-gray-500">
              Wenn du &quot;Behalten&quot; klickst, wird das Bild im Bilder-Block
              dieses Posts gespeichert.
            </p>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                id="format"
                value={sizeFormat}
                onChange={(e) => setSizeFormat(e.target.value as SizeFormat)}
                disabled={isGenerating}
                className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs"
              >
                <option value="auto">{autoFormatLabel}</option>
                <option value="1024x1024">{FORMAT_LABELS["1024x1024"]}</option>
                <option value="1536x1024">{FORMAT_LABELS["1536x1024"]}</option>
                <option value="1024x1536">{FORMAT_LABELS["1024x1536"]}</option>
              </select>
            </div>
            ...
            <div>
              ...
              <Input
                ...
                placeholder="z.B. 'wärmere Töne', 'mehr Aquarell', 'mit Bergblick'"
                disabled={isGenerating}
              />
            </div>
            ...
            <div className="rounded-lg bg-[#F8F6F3] dark:bg-gray-800 p-3 text-xs text-[#5E5346] dark:text-gray-400">
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  Format wird automatisch zur Plattform passend gewählt.
                </div>
              </div>
            </div>
